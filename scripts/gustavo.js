...
class Rectangle {
  ...
  draw() {
    if (movingImage) {
      imageMode(CENTER);
      image(movingImage, this.x, this.y, movingImage.width / 2.5, movingImage.height / 2.5);
    } else {
      fill(this.color);
      stroke(0);
      strokeWeight(5);
...
class Obrazek {
  ...
  draw() {
    if (collectImage) {
      imageMode(CENTER);
      image(collectImage, this.x, this.y, this.size * 8, this.size * 8);
    } else {
      fill(this.color);
...
function preload() {
  movingImage = loadImage('../pictures/gustavo.png');
  collectImage = loadImage('../pictures/chicken.webp');
  collisionSound = loadSound('../sounds/yummy.mp3');
  backgroundImage = loadImage('../pictures/lospollos.jpg');
...
function draw() {
  ...
  text("The goal of the game is to collect " + end + " chicken sandwiches.", width / 2, 20);
